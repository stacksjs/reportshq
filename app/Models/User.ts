...
export default defineModel({
  ...
  traits: {
    ...
    // ReportsHQ bills through the User: plans are per account, not per
    // project, so checkout(), activeSubscription() and the rest of
    // createBillableMethods hang off this model. See #17.
    billable: true,
    useUuid: true,
    ...
    // No `useApi`. The generated CRUD is not scoped to the requesting user,
    // so `index` would list every account on the instance and `show` would
    // read any of them by id. There is nothing an authenticated user needs
    // from another user's record here; the session's own account is served by
    // the account routes instead.
  },
